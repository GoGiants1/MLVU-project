...
def get_args_parser():
    ...
    parser.add_argument(
        "--existing_fgmask_input",
        type=str,
        default="./datasets/HierText/val_fgmask/",
    ...
    parser.add_argument(
        "--eval_out_file",
        type=str,
        default="./hiertext_eval/res_per_img",
        help="A file or directory to save results per image.",
    )
    parser.add_argument(
        "--total_points", default=1500, type=int, help="The number of foreground points"
    )
    parser.add_argument(
        "--batch_points", default=100, type=int, help="The number of points per batch"
    )
        ...
        if args.eval:
            if masks is None:
                lines = [
                    {
                        "words": [
                            {"text": "", "vertices": [[0, 0], [1, 0], [1, 1], [0, 1]]}
                        ],
                        "text": "",
                    }
                ]
                paragraphs = [{"lines": lines}]
                result = {"image_id": img_id, "paragraphs": paragraphs}
                none_num += 1
            else:
                ...
                result = {"image_id": img_id, "paragraphs": paragraphs}
            with open(
                os.path.join(args.eval_out_file, img_id + ".jsonl"),
                "w",
                encoding="utf-8",
            ) as fw:
                json.dump(result, fw)
