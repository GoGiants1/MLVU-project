...
args = Args(
    model_type="vit_h",
    checkpoint="/root/inpaintanything/hi_sam/pretrained_checkpoint/hi_sam_h.pth",
    dataset="ctw1500",
...
def making_mask(img):
    ...
    final_mask=masks.squeeze(1)
    mask=np.zeros_like(final_mask[0])
    for i in range(final_mask.shape[0]):
        mask+=final_mask[i]
        
    final_mask2=mask
    final_mask2=Image.fromarray(final_mask2)
    
    return final_mask2

# masks=making_mask("/root/inpaintanything/samples/scene1.jpg")
# mask=masks[1]
# mask=mask.squeeze(0)
# mask=Image.fromarray(mask)
# mask.save("out_scene1_2.jpg")
