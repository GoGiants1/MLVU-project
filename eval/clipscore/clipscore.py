...
import pathlib
import json
from .generation_eval_utils import *
import pprint
import warnings
...
class CLIPImageDataset(torch.utils.data.Dataset):
    ...
    def _transform_test(self, n_px):
      ...
      #self.data : numpy array (b, w, h, channel)
    def __getitem__(self, idx):
        c_data = self.data[idx]
        #im = Image.fromarray((x * 255).astype(np.uint8))

        #image = Image.fromarray(c_data.astype(np.uint8)) clip score 58
        image = Image.fromarray((c_data*255).astype(np.uint8)) #clip score 84
        #위 이미지는 pillow array
...
def cal_clip_score(img_list,caption_list):
    

    image_paths = img_list 
    #image_paths : numpy array (b,w,h,c)
    
    candidates =caption_list
    ...
    scores = {f"image{index+1}": {'CLIPScore': float(clipscore)}
                for index,clipscore in enumerate(per_instance_image_text)}
    #print(scores)
    final_score=np.mean([s['CLIPScore'] for s in scores.values()])
...
if __name__ == '__main__':
    list1=["example/images/image1.jpg","example/images/image2.jpg"]
    list2=["an orange cat and a grey cat are lying together.","a black dog wearing headphones looks at the camera as an orange cat walks in the background."]
    s=cal_clip_score(list1,list2)
    print(s)
