...
try:
    ...
    def tqdm(x):
        return x
import sys
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'pytorch_fid')))
...
class ImagePathDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, i):
        ...
        img = Image.fromarray((path*255).astype(np.uint8)).convert("RGB")
...
def compute_statistics_of_path(path, model, batch_size, dims, device, num_workers=1):
    ...
    m, s = calculate_activation_statistics(
        array, model, batch_size, dims, device, num_workers
    )
...
def cal_fid(pred_array,target_array):
    #위둘은 numpy array
    
    device = torch.device("cuda")
    
    num_workers = 4
    #pred_array (b,w,h,3) 3은 채널  
    fid_value = calculate_fid_given_paths(
        [pred_array,target_array], pred_array.shape[0], device, 2048, num_workers
    )
    return fid_value
...
if __name__ == "__main__":
    ...
    batch=20 #vis num, num of samples

    pred_array=torch.rand(batch,3,100,100)
    target_array=torch.rand(batch,3,100,100)
    pred_array=torch.permute(pred_array,(0,2,3,1))
    target_array=torch.permute(target_array,(0,2,3,1))

    pred_array=np.array(pred_array)
    target_array=np.array(target_array)

    print(cal_fid(pred_array,target_array))
