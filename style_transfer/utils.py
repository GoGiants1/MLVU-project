...
def load_config(config):
    activate_layer_indices_list = config["inference_info"][
...
def get_canny_edge_array(canny_img_path, threshold1=100, threshold2=200):
    ...
    if os.path.isdir(canny_img_path):
        ...
        for canny_img in canny_img_list:
            ...
            canny_image_tmp = np.concatenate(
                [canny_image_tmp, canny_image_tmp, canny_image_tmp], axis=2
            )
...
def get_depth_edge_array(
    depth_img_path, feature_extractor, depth_estimator, device="cuda"
):
    depth_image_list = []
    ...
    if os.path.isdir(depth_img_path):
        ...
        for depth_img in depth_img_list:
            depth_image_tmp = Image.open(
                os.path.join(depth_img_path, depth_img)
            ).convert("RGB")

            # get depth map
            depth_map = get_depth_map(
                depth_image_tmp, feature_extractor, depth_estimator, device
            )
