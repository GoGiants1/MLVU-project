...
parser.add_argument("--depth_img_path", type=str, default="assets/depth_dir")
...
if __name__ == "__main__":
    ...
    depth_image_list = get_depth_edge_array(
        args.depth_img_path, feature_extractor, depth_estimator, device
    )
    depth_image_name_list = os.listdir(args.depth_img_path)

    for d_i, depth_image in enumerate(depth_image_list):
        depth_image_name_wo_ext = depth_image_name_list[d_i].split(".")[0]
                    ...
                    for activate_layer_indices in activate_layer_indices_list:
                        ...
                        for activate_step_indices in activate_step_indices_list:
                            ...
                            for ref_seed in ref_seeds:
                                ...
                                # save_name = f"ref_{ref_object}_inf_{inf_object}_activate_layer_{str_activate_layer}_ref_seed_{ref_seed}_inf_seed_{inf_seeds}_wo_lora.png"
                                save_name = f"depth_{depth_image_name_wo_ext}_control-scale_{str_controlnet_scale}_style_{style_name}_ref_{ref_seed}_{ref_object}_inf_{new_inf_seeds}_{inf_object}activate_layer_{str_activate_layer}_step_{str_activate_step}.png"
