...
class ResizeLongestSide_ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        image, shape = sample["image"], sample["shape"]  # noqa: F841
        image = torch.as_tensor(image, dtype=torch.float32)
        image = image.permute(2, 0, 1)
        target_size = self.get_preprocess_shape(
            image.shape[1], image.shape[2], self.target_length
        )
        image = torch.squeeze(
            F.interpolate(torch.unsqueeze(image, 0), target_size, mode="bilinear"),
            dim=0,
        )
...
class OnlineDataset(Dataset):
    def __init__(
        self, name_im_gt_list, transform=None, eval_ori_resolution=False, hier_det=False
    ):
        ...
        im_name_list.extend(
            [
                x.split(os.sep)[-1].split(name_im_gt_list["im_ext"])[0]
                for x in name_im_gt_list["im_path"]
            ]
        )
        ...
        if hier_det:
            ...
            self.dataset["annotations"] = {
                anns["image_id"]: anns for anns in annotations
            }
            print(
                f"processed {len(self.dataset['annotations'])} samples from {json_path} in {time.time()-load_start:.2f} seconds."
            )
            del annotations
    ...
    def __getitem__(self, idx):
        ...
        im_name = self.dataset["im_name"][idx]  # noqa: F841
        ...
        if "TextSeg" in self.dataset_name:
            ...
            gt = (gt_ori > 127).astype(
                np.uint8
            ) * 255  # for TotalText, HierText: 0 or 255
        if len(gt.shape) > 2:
            gt = gt[:, :, 0]
        if len(im.shape) < 3:
            im = im[:, :, np.newaxis]
        ...
        if self.eval_ori_resolution:
            ...
            if "TextSeg" in self.dataset_name:
                ...
                sample["ignore_mask"] = torch.unsqueeze(
                    torch.from_numpy(ignore_mask), 0
                )
