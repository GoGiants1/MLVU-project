...
def process_caption(font_path, caption, keywords):
    ...
    for keyword in keywords:
        ...
        if keyword in caption_split:
            if keyword not in duplicate_dict:
                ...
                if (
                    duplicate_dict[keyword] + 1 < len(caption_split)
                    and keyword in caption_split[duplicate_dict[keyword] + 1 :]
                ):
                    index = duplicate_dict[keyword] + caption_split[
                        duplicate_dict[keyword] + 1 :
                    ].index(keyword)
    ...
    return (
        ...
        length_list,
        width_list,
        torch.from_numpy(info_array),
        words,
        torch.Tensor(state_list).long(),
        torch.Tensor(word_match_list).long(),
        torch.Tensor(boxes),
        boxes_length,
    )
...
def get_layout_from_prompt(args):
    ...
    print(
        f'{colored("[!]", "red")} Detected keywords: {keywords} from prompt {args.prompt}'
    )
    ...
    (
        ...
        boxes,
        boxes_length,
    ) = process_caption(font_path, args.prompt, keywords)
    ...
    with torch.no_grad():
        for box_index in range(boxes_length):
            ...
            output, encoder_embedding = model(
                text_embedding,
                length_list,
                width_list,
                mask,
    ...
    for index, box in enumerate(return_boxes):
        ...
        for i, char in enumerate(text):
            ...
            segmentation_mask_draw.rectangle(
                shrink_box(char_box, scale_factor=0.9), fill=char_index
            )
