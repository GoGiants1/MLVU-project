...
class TextConditioner(nn.Module):
    ...
    def forward(self, prompt_list):
        ...
        return (
            text_embedding.last_hidden_state.cuda(),
            batch_encoding["attention_mask"].cuda(),
        )  # 1, 77, 768  /  1, 768
    ...
    def forward(
        ...
        if encoder_embedding is None:
            ...
            if train:
                width = (
                    width
                    + torch.randint(-3, 3, (width.shape[0], width.shape[1])).cuda()
                )

            x = self.input_layer(x)  # (1, 77, 512)
            width_embedding = self.width_embedding(
                torch.clamp(width, 0, 255).long()
            )  # (1, 77, 512)
            encoder_target_embedding = self.encoder_target_embedding(
                target[:, :, 0].long()
            )  # (1, 77, 512)
            pe_embedding = self.position_embedding(torch.arange(77).cuda()).unsqueeze(
                0
            )  # (1, 77, 512)
            total_embedding = (
                x + width_embedding + pe_embedding + encoder_target_embedding
            )  # combine all the embeddings (1, 77, 512)
        ...
        right_shifted_boxes_resize = torch.clamp(
            right_shifted_boxes_resize, 0, 511
        )  # (1, 8, 4)

        # decoder pe
        pe_decoder = torch.arange(8).cuda()  # (8, )
        pe_embedding_decoder = self.position_embedding(pe_decoder).unsqueeze(
            0
        )  # (1, 8, 512)
        decoder_input = (
            pe_embedding_decoder
            + self.x_embedding(right_shifted_boxes_resize[:, :, 0])
            + self.y_embedding(right_shifted_boxes_resize[:, :, 1])
            + self.w_embedding(right_shifted_boxes_resize[:, :, 2])
            + self.h_embedding(right_shifted_boxes_resize[:, :, 3])
        )  # (1, 8, 512)
        ...
        mask = mask.cuda()  # (8, 8)
        decoder_result = self.decoder_transformer(
            decoder_input, encoder_embedding, tgt_mask=mask
        )  # (8, 1, 512)
