...
def segmentation_mask_visualization(font_path: str, segmentation_mask: np.array):
    ...
    segmentation_mask = cv2.resize(
        segmentation_mask, (64, 64), interpolation=cv2.INTER_NEAREST
    )
    ...
    for i in range(64):
        for j in range(64):
            if int(segmentation_mask[i][j]) == 0 or int(
                segmentation_mask[i][j]
            ) - 1 >= len(alphabet):
                continue
            else:
                d.text(
                    (j * 8, i * 8),
                    alphabet[int(segmentation_mask[i][j]) - 1],
                    font=font,
                    fill=(0, 255, 0),
                )
    return blank
...
def combine_image(
    args,
    sub_output_dir: str,
    pred_image_list: List,
    image_pil: Image,
    character_mask_pil: Image,
    character_mask_highlight_pil: Image,
    caption_pil_list: List,
):
    """
