...
def parse_args():
    ...
    parser.add_argument(
        "--vis_interval", type=int, default=500, help="The interval for visualization."
    )
...
def main():
    ...
    if version.parse(accelerate.__version__) >= version.parse("0.16.0"):
        ...
        def save_model_hook(models, weights, output_dir):
            if args.use_ema:
                ema_unet.save_pretrained(os.path.join(output_dir, "unet_ema"))  # noqa
        ...
        def load_model_hook(models, input_dir):
            if args.use_ema:
                ...
                ema_unet.load_state_dict(load_model.state_dict())  # noqa
                ema_unet.to(accelerator.device)  # noqa
                del load_model
    ...
    def preprocess_train(examples):
        ...
        for image in examples[image_column]:
            ...
            if args.no_pos_con:
                segmentation_mask = (
                    np.load(f"{args.dataset_path}/{first}/{second}/charseg.npy") * 0
                )
            elif args.no_con:
                segmentation_mask = (
                    np.load(f"{args.dataset_path}/{first}/{second}/charseg.npy") > 0
                ...
                segmentation_mask = np.load(
                    f"{args.dataset_path}/{first}/{second}/charseg.npy"
                )
            ...
            if args.segmentation_mask_aug:  # 10% dilate / 10% erode / 10% drop
                ...
                if random_value < 0.6:
                    ...
                    segmentation_mask = cv2.dilate(
                        segmentation_mask.astype(np.uint8), kernel, iterations=1
                    )
                    ...
                    kernel = np.ones((3, 3), dtype=np.uint8)
                    segmentation_mask = cv2.dilate(
                        segmentation_mask.astype(np.uint8), kernel, iterations=2
                    )
                    ...
                    segmentation_mask[drop_mask] = (
                        0  # set character to non-character with 10% probability
                    )
    ...
    def collate_fn(examples):
        ...
        image_masks = torch.cat(
            [example["image_masks"].unsqueeze(0) for example in examples], 0
        )
        segmentation_masks = torch.cat(
            [
                torch.from_numpy(example["segmentation_masks"])
                .unsqueeze(0)
                .unsqueeze(0)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                if accelerator.is_main_process:
                    if (step + 1) % args.vis_interval == 0:
                        ...
                        for t in tqdm(scheduler.timesteps):
                            with torch.no_grad():
                                noisy_residual = unet(
                                    input,
                                    t,
                                    encoder_hidden_states[: args.vis_num],
                                    masked_feature=masked_features[:16],
                                    feature_mask=feature_masks[:16],
                                    segmentation_mask=segmentation_masks[:16],
                                ).sample
                        ...
                        new_image.save(
                            f"{args.output_dir}/[{epoch}]_{(step + 1) // args.vis_interval}_pred_img.png"
                        )
                        ...
                        for index, image in enumerate(
                            segmentation_masks[: args.vis_num]
                        ):
                            segmap_pil = Image.fromarray(
                                ((image != 0) * 255)
                                .squeeze()
                                .cpu()
                                .numpy()
                        ...
                        for index, image in enumerate(batch["images"][: args.vis_num]):
                            ...
                            new_image.paste(image, (col * width, row * height))
                        new_image.save(
                            f"{args.output_dir}/[{epoch}]_{(step + 1) // args.vis_interval}_orig_img.png"
                        )
                ...
                resized_charmap = F.interpolate(
                    batch["segmentation_masks"].float(), size=(64, 64), mode="nearest"
                ).long()

                ce_loss = ce_criterion(
                    segmenter(pred_x0.float()), resized_charmap.squeeze(1)
                )
                mse_loss = F.mse_loss(
            ...
            if accelerator.sync_gradients:
                if args.use_ema:
                    ema_unet.step(unet.parameters())  # noqa
    ...
    if accelerator.is_main_process:
        ...
        if args.use_ema:
            ema_unet.copy_to(unet.parameters())  # noqa
