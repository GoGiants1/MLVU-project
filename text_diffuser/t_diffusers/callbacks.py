...
class PipelineCallback(ConfigMixin):
    ...
    def __init__(self, cutoff_step_ratio=1.0, cutoff_step_index=None):
        ...
        if (cutoff_step_ratio is None and cutoff_step_index is None) or (
            ...
            raise ValueError(
                "Either cutoff_step_ratio or cutoff_step_index should be provided, not both or none."
            )

        if cutoff_step_ratio is not None and (
            not isinstance(cutoff_step_ratio, float)
            or not (0.0 <= cutoff_step_ratio <= 1.0)
        ):
    ...
    def tensor_inputs(self) -> List[str]:
        raise NotImplementedError(
            f"You need to set the attribute `tensor_inputs` for {self.__class__}"
        )

    def callback_fn(
        self, pipeline, step_index, timesteps, callback_kwargs
    ) -> Dict[str, Any]:
        raise NotImplementedError(
            f"You need to implement the method `callback_fn` for {self.__class__}"
        )
...
class MultiPipelineCallbacks:
    ...
    def tensor_inputs(self) -> List[str]:
        return [
            input for callback in self.callbacks for input in callback.tensor_inputs
        ]
